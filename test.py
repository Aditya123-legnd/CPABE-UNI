...
if __name__ == '__main__':
    sourcefile = open("uni_record.json", 'rb')
    msg = sourcefile.read()
    sourcefile.close()
    msg_dict = json.loads(msg)
    policy = '((' + msg_dict["ID"] + ') or (' 
    for item in msg_dict['FACULTY_INCHARGE']:
        if msg_dict['FACULTY_INCHARGE'][-1] != item:
            policy += "(" + item['ID'] + ' and ' + item['Faculty'].upper() + ")" + " or "
        else:
            policy += "(" + item['ID'] + ' and ' + item['Faculty'].upper() + ")" + '))'
